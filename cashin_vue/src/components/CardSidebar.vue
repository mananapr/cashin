      ...
      >
        ...
        <li>
          <button
            ...
            aria-controls="dropdown-income"
            data-collapse-toggle="dropdown-income"
          >
          ...
          <ul id="dropdown-income" class="py-2 space-y-2">
            <li v-for="income in incomeOptions" :key="income.id">
              <div class="flex items-start mb-6 ml-8">
        ...
        <li>
          <button
            type="button"
            class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
            aria-controls="dropdown-rewards"
            data-collapse-toggle="dropdown-rewards"
          >
            <span class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap"
              >Rewards</span
            >
          ...
          <ul id="dropdown-rewards" class="py-2 space-y-2">
            <li v-for="reward in rewardOptions" :key="reward.id">
              <div class="flex items-start mb-6 ml-8">
                <div class="flex items-center h-5">
                  <input
                    :id="reward.id"
...
const { updateCardQuery } = useFilterList()

const incomeOptions = [
  { id: '5lpa', label: '5LPA' },
  { id: '10lpa', label: '10LPA' },
  { id: '15lpa', label: '15LPA' },
  { id: '20lpa', label: '20LPA' },
  { id: '25lpa', label: '25LPA' },
  { id: '30lpa', label: '30LPA' },
]
...
const fetchIssuers = async () => {
  loading.value = true
  try {
    const resultList = await pb.collection('card_issuers').getFullList()
...
const applyFilters = () => {
  ...
  if (filters.income.length) {
    let maxIncome = 0
    for (i = 0; i < filters.income.length; i++) {
      let income = filters.income[i]
      income = parseInt(income.replace('LPA', ''))
      if (income > maxIncome) {
        maxIncome = income
      }
    }
    queryFilters.push(`(monthly_income <= ${maxIncome})`)
  }

  if (filters.rewards.length) {
    const rewardFilters = []
    for (i = 0; i < filters.rewards.length; i++) {
      rewardFilters.push(`labels ~ "${filters.rewards[i]}"`)
    }
    queryFilters.push(`(${rewardFilters.join(' || ')})`)
  }
  ...
  updateCardQuery(queryFilters.join(' && '))
}
