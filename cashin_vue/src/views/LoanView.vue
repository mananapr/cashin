...
import LoanSidebar from '../components/LoanSidebar.vue'
import useFilterList from '../composables/useFilterList.js'

const loading = ref(false)
const loans = ref([])
const noResults = ref(false)
const { loanQuery } = useFilterList()
  ...
  try {
    if (loanQuery.value === '') {
      const resultList = await pb
        .collection('products')
        .getFullList({ filter: 'product_type="loan"', sort: '-priority' })
      loans.value = resultList
    } else {
      const resultList = await pb
        .collection('products')
        .getFullList({
          filter: `(product_type="card") && ${loanQuery.value}`,
          sort: '-priority',
        })
      loans.value = resultList
    }
